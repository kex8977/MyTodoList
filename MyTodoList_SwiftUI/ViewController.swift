...
class ViewController: UIViewController, UITableViewDataSource, UITableViewDelegate {
  ...
  // var animation : LottieAnimation?
   //var animationView = LottieAnimationView(animation: animation)
    ...
    override func viewDidLoad() {
       ...
       let animation = LottieAnimation.named("movingCar")
       let animationView = LottieAnimationView(animation: animation)
       
       animationView.frame = CGRect(x: 16, y: 600, width: 361, height: 230)
       animationView.loopMode = .loop // 애니메이션 무한 재생
...
//         animation = LottieAnimation.named("movingCar")
      }
      else{
         sender.setTitle("Edit", for: .normal)
         tableViewEx.isEditing = false
         
//         animation = LottieAnimation.named("scan_IDcard")
      }
   }
}
